...
- `internal/service/`:
  ...
  - `auth.go` — `Login(ctx, db, email, password)`: `GetUserByEmail` → `bcrypt.CompareHashAndPassword`; unknown-email and wrong-password both collapse to `ErrInvalidCredentials` (no enumeration). DB-only — the HTTP layer mints the token.
...
- `internal/http/` (package `httpapi` — directory named `http` but package renamed to avoid stdlib clash):
  - `server.go` — `*Server` struct (db, logger, auth, mux), `NewServer(db, logger, authenticator)`, `routes()` (registers all 12 routes), `ServeHTTP` (delegates to mux), `Run(ctx, addr, shutdownTimeout)` with graceful shutdown via goroutine + `select` on `ctx.Done()` vs. error channel.
  ...
  - Routes registered: `POST /signup` (public), `POST /login` (public), `POST /accounts`, `GET /accounts`, `GET /accounts/{id}/balance`, `POST /accounts/{id}/{close|freeze|reopen}`, `POST /transactions/{transfer|deposit|withdraw|assess-fee|refund-fee}`, `POST /transactions/{id}/reverse`.
...
- `main.go` opens DB → runs migrations → builds `slog.Logger` (JSON handler) → reads `JWT_SECRET` and builds `auth.New(secret, TOKEN_TTL = 15m)` (**fatal if unset**) → builds signal-cancellable ctx via `signal.NotifyContext` → `httpapi.NewServer(database, logger, authenticator)` → `srv.Run(ctx, ":8080", SHUTDOWN_TIME)`. DB connection string still inline (env-var TODO unchanged).
...
- **Admin bootstrap.** Nothing can mint an admin: `signup` always assigns `RoleUser`, and no endpoint or seed sets `role = ADMIN`. So the four admin routes (deposit, assess-fee, refund-fee, reverse) are correctly gated but **uncallable until an admin exists**. Pick a path: manual `UPDATE users SET role='ADMIN' WHERE email=...`, a seed migration for a bootstrap admin, or an admin-management endpoint.
